...
public class HelperFunctions{
	...
	public static double battlePortalCalc(double x, double y, LinkedList<Planet> portals, int portalsResearch){
		double cover = 0;		
		for (Planet portal : portals){
				double min_dist = Math.pow(((double)portal.x - x),2) + Math.pow(((double)portal.y - y),2);
			    double dist = Math.pow(((double)portal.x - x),2) + Math.pow(((double)portal.y - y),2);
			    if (dist < min_dist){
				    min_dist = dist;
				}
		        double d = Math.sqrt(Math.pow((x-portal.x),2.0) + Math.pow((y-portal.y),2.0));
		        cover = Math.max(0, 1.0 - Math.sqrt(d/(10.0*(1.0 + 0.01*portalsResearch))));
		}
		return cover;
	}
