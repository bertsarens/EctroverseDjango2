...
def headquarters(request):
    ...
    context = {"status": status,
               ...
               "artis": Artefacts.objects.filter().first}
...
def smap(request):
    ...
    for p in planets:
        ...
        if p.x != prevx or p.y != prevy:
            if count==pcount and count!=0:
                print(str(prevx)+","+str(prevy)+" fully scanned:"+str(pcount))
                scoutgen.append({"s": setting.map_setting, "x": prevx, "y": prevy, "color":hex_format(sum_tuple(scout_color,scout_color))})
        ...
        for setting in settings:
            ...
            if setting.map_setting == "YR" and porcount == 0:
                if p.owner == status.user and p.portal == True:
                    porcount += 1
                    scoutgen.append({"s": setting.map_setting, "x": p.x, "y": p.y, "color":hex_format(sum_tuple(color,color))})
...
def planets(request):
    ...
    if order_by == 'planet':
        planets = Planet.objects.filter(owner=request.user).exclude(i=9).order_by('x', 'y', 'i')
    else:
        planets = Planet.objects.filter(owner=request.user).exclude(i=9).order_by(order_by)  # directly use the keyword
...
def planet(request, planet_id):
    ...
    ops = Specops.objects.filter(user_to=status.user, name="Dark Web")
    specops=[]
    for op in ops:
        specops.append(specops)	
...
def system(request, system_id):
    ...
    planet = Planet.objects.filter(x=system.x, y=system.y, owner=None)
    main_fleet = Fleet.objects.get(owner=request.user, main_fleet=True)
    msg = ""
    if request.method == 'POST' and 'explore_all' in request.POST:
        ...
        for p in planet: 
            if main_fleet.exploration == 0:
                ...
                best_portal_planet = find_nearest_portal(p.x, p.y, portal_planets)
        ...
        for p in planet:
            ...
            if main_fleet.agent < agents:
                ...
                Fleet.objects.create(owner=status.user,
                     ...
                     current_position_y=best_portal_planet.y,
                     agent=agents,
                     specop="Observe Planet")
                msg += "\nAgents sent to " + str(p.x) + "," + str(p.y) + ":" + str(p.i)
    ...
    context = {"status": status,
               ...
               "settings": MapSettings.objects.filter(user=status.id), 
               "planets": Planet.objects.filter(x=system.x, y=system.y),
               "yp": Planet.objects.filter(owner=status.user, x=system.x, y=system.y),
               "msg": msg,
               "page_title": "System " + str(system.x) + ',' + str(system.y)}
...
def mass_build(request):
    ...
    if 'planets_id_mass_build' in request.POST:
        ...
        for pid in planets_id:
            ...
            totalbuild -= planet.total_buildings
...
def ranking(request):
    status = get_object_or_404(UserStatus, user=request.user)
    table = UserStatus.objects.exclude(user_name='').order_by('-num_planets', '-networth')
    context = {"table": table,
...
def empire_ranking(request):
    ...
    empartis = Artefacts.objects.filter(empire_holding=status.empire).count()
...
def fleetsend(request):
    ...
    race = status.race
    if order == 0 and race == "SB" and best_portal_planet.x + 2 >= x >= best_portal_planet.x - 2 and best_portal_planet.y + 2 >= y >= best_portal_planet.y - 2:
        fleet_time = 0
    elif order == 0 and best_portal_planet.x + 1 >= x >= best_portal_planet.x - 1 and best_portal_planet.y + 1 >= y >= best_portal_planet.y - 1:
        fleet_time = 0
    else:
...
def empire(request, empire_id):
    ...
    context = {"status": status,
               ...
               "empire": empire1,
               "artefacts": Artefacts.objects.filter(empire_holding=empire1),
               "artis": Artefacts.objects.filter(empire_holding=empire_id).count(),
               "articount": articount
               }
